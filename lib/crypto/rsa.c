	...
	const uint8_t *hash)
{
	...
	if (sig_len != (key->len * sizeof(uint32_t))) {
		debug("RSA: Signature is of incorrect length!\n");
		return 0;
	}

	if (sig_type >= kNumAlgorithms) {
		debug("RSA: Invalid signature type!\n");
		return 0;
	}

	if (key->len != siglen_map[sig_type] / sizeof(uint32_t)) {
		debug("RSA: Wrong key passed in!\n");
		return 0;
	}
	...
	* SafeMemcmp() just to be on the safe side. */
	...
	if (SafeMemcmp(buf, padding, padding_len)) {
		debug("RSA: In RSAVerify(): Padding check failed!\n");
		success = 0;
	}

	/* Check hash. */
	if (SafeMemcmp(buf + padding_len, hash, sig_len - padding_len)) {
		debug("RSA: In RSAVerify(): Hash check failed!\n");
		success  = 0;
	}
	free(buf);
